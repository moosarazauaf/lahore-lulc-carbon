...
def main():
    ...
    areas = {y: markov.area_hectares(maps[y], mask) for y in OBSERVED_YEARS}
    ...
    for a, b in zip(OBSERVED_YEARS, OBSERVED_YEARS[1:]):
        ...
        print(f"\n  TRANSITIONS {a} -> {b}")
        print(markov.format_matrix(counts))
    ...
    results.append(validate.report(maps[2003], maps[2013], sim13[2013], mask,
                                   "calibrated 1993-2003, predicted 2013"))
    print("\n" + validate.per_class_area_error(maps[2013], sim13[2013], mask))

    sim23, *_ = calibrate_and_simulate(maps, mask, 2003, 2013, 2013, [2023])
    results.append(validate.report(maps[2013], maps[2023], sim23[2023], mask,
                                   "calibrated 2003-2013, predicted 2023"))
    print("\n" + validate.per_class_area_error(maps[2023], sim23[2023], mask))
    ...
    for y, arr in proj.items():
        ...
        areas[y] = markov.area_hectares(arr, mask)
    ...
    bounds = {y: carbon.stock_with_bounds(all_maps[y], mask) for y in all_years}
    legacy = {y: carbon.legacy_stock(all_maps[y], mask) for y in all_years}
    ...
    for a, b in ((1993, 2023), (2023, 2043)):
        rows = carbon.change_by_transition(all_maps[a], all_maps[b], mask)
